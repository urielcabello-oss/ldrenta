document.addEventListener("DOMContentLoaded", function () {
  const modalinfounidademojefedirecto = new bootstrap.Modal(document.getElementById("modalinfounidademojefedirecto"));
  const modalinfounidademojefedirectobody = document.getElementById("modalinfounidademojefedirectobody");
  ...
  document.body.addEventListener("click", function (event) {
    if (event.target.classList.contains("btnMosrarModalUnidad")) {
      ...
      $.ajax({
        type: "POST",
        url: "../../Servidor/solicitudes/solicitud_unidades_demo/formularioautorizarunidadjefedirecto.php",
        ...
        success: function (response) {
          modalinfounidademojefedirectobody.innerHTML = response;
          modalinfounidademojefedirecto.show();
        },
      });
    }
  });
  ...
  // Autorizar unidad
  document.body.addEventListener("click", function (event) {
    if (event.target && event.target.id === "btnaprovarunidademojefedirecto") {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          $.ajax({
            type: "POST",
            url: "../../Servidor/solicitudes/solicitud_unidades_demo/autorizar_unidad_demo_jefe_directo.php",
            data: formData,
            contentType: false,
            processData: false,
            success: function (response) {
              ...
              if (response.includes("correctamente")) {
                window.location.href = "./solicitudes_por_autorizar_jefe.php?resultado=Autorizacionunidademo";
              } else {
                Toastify({
