...
WHERE asigun.id_asignacion_unidad_demo = '$idasignacion'";
  ...
  $resultado = $conexion->query($queryobtenerinfounidadasignacion);
  ...
  if ($fila = $resultado->fetch_assoc()) {
    ...
    $solicitar_emplacamiento_ldr = $fila['solicitar_emplacamiento_ldr'] ?? 0;
    $solicitar_seguro_ldr = $fila['solicitar_seguro_ldr'] ?? 0;
    $vin = $fila['vin'];
    $numero_motor = $fila['numero_motor'];
    $placa = $fila['placa'];
    $tarjeta_circulacion = $fila['costo_neto'];
    $color = $fila['color_unidad'];
    $ubicacion = $fila['ubicacion'] ?? 'Sin sede';
  }
...
?>
<?php if ($solicitar_emplacamiento_ldr == 1): ?>
  <div class="contenedorplaca mt-3">
    <h4 for="nueva_placa_demo" class="form-label">Placa asignada por LDR</h4>
    <input type="text" class="form-control nueva_placa_demo" id="nueva_placa_demo" name="nueva_placa_demo"
      pattern="[A-Z0-9]{6,8}" title="Debe tener entre 6 y 8 caracteres alfanuméricos"
      placeholder="Ingresa la placa">
    <input type="hidden" name="id_unidad" value="<?php echo $fila['id_unidad']; ?>">
  </div>
<?php endif; ?>